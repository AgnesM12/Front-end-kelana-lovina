    ...
    <main className="w-full max-w-7xl mx-auto px-6 sm:px-8 mt-16 overflow-x-hidden">
      ...
      <div className="mt-8">
        {filtered.length > 0 ? (
          ...
          <p className="text-center text-gray-500 m-60">Tidak ada ulasan yang cocok</p>
        )}
      </div>
    </main>
  );
  ...
  return (
    <div className="w-96 p-6 bg-white rounded-[30px] shadow-[0px_6px_40px_0px_rgba(0,94,209,0.16)] flex flex-col items-center gap-6">
      ...
      <div className="flex flex-col gap-2">
        <h3 className="w-[340px] text-zinc-700 text-2xl font-semibold">{aktivitas.title}</h3>
        <p className="w-80 text-gray-600 text-xl font-normal pb-6">{aktivitas.desk}</p>
        <button
          disabled={disabled}
          onClick={() =>
            !disabled &&
